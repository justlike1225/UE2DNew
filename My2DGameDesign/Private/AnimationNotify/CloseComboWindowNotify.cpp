#include "AnimationNotify/CloseComboWindowNotify.h" // 包含对应的头文件
...
void UCloseComboWindowNotify::OnReceiveNotify_Implementation(UPaperZDAnimInstance* OwningInstance) const
{
	...
	if (!Hero)
	{
		UE_LOG(LogTemp, Warning, TEXT("CloseComboWindowNotify: 无法从动画实例获取有效的 SpriteHero 角色。"));
		return;
	}
	...
	if (!CombatComp)
	{
		UE_LOG(LogTemp, Warning, TEXT("CloseComboWindowNotify: 角色身上没有找到 HeroCombatComponent。"));
		return;
	}

	// 调用战斗组件的函数来关闭连击窗口并设置重置检查
